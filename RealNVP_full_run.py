...
def main():
    ...
    showers_raw, cs, E_inc, fine_flat, cond_flat, factors = Preprocess2.build_and_cache(
        ...
        cache_file = "/teo_fs_fast/users/acosso/Dataset/full_preproc_A.hdf5",
        mode       = MODE,
    ...
    if MODE == 'A':
        ndims = 10
        if layer_energies: # remove the fine layer energies 
            ncond = 31
        else: 
            ncond = 26
    else: 
    ...
    RealNVP = Bijectors.ChooseCondBijector(bijector_name, ndims, spline_knots, 
    ...
    schedule_mom = OneCycleSchedule(1e-3, steps_per_epoch,
                                ...
                                annihilation_epochs = annihilation_epochs,
                                momentum            = True,           # << flip!
                                max_mom=0.95, min_mom=0.88
    )

    '''
    schedule_lr = OneCyclePlateau(steps_per_epoch=steps_per_epoch,
                            warmup_epochs=10,          # base -> max
                            ramp_epochs=25,            # max -> plateau (cosine)
                            plateau_epochs=10,        # <-- flat
                            cooldown_epochs=30,       # plateau -> base (cosine)
